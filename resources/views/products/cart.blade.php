                    ...
                    <div class="col-md-8">
                        <div class="card mb-4">
                            <div class="card-header py-3">
                                <h5 class="mb-0" id="cart-count">Cart -
                                    {{ count($cart) <= 1 ? count($cart) . ' item' : count($cart) . ' items' }}</h5>
                            </div>
                            @unless (count($cart) == 0)
                                    ...
                                    <div class="card-body" id="cart-item_{{ $item->product_id }}">
                                        <div class="row">
                                            <div
                                                ...
                                                <div class="w-50 p-2">
                                                    <img src="{{ asset('storage/' . $item->image_path) }}" class=""
                                                        alt="" />
                                                </div>
                                            </div>
                                            ...
                                            <div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
                                                <div class="d-flex mb-4" style="max-width: 300px">
                                                    <div>
                                                        <button class="btn btn-primary mx-1"
                                                            onclick="decrement({{ $item->product_id }}, {{ $item->id }}, {{ $item->price }})"
                                                            id="decrementButton_{{ $item->product_id }}"
                                                            class="bg-blue-500 {{ $item->quantity <= 1 ? ' bg-gray-500' : '' }}"
                                                    ...
                                                    <div class="d-flex justify-content-center text-center">
                                                        <div class="form-outline">
                                                            <input class="text-center form-control" type="number"
                                                                ...
                                                                value={{ $item->quantity }} min="1" readonly disabled />
                                                ...
                                                <p class="text-center text-md-center">
                                                    ₱ <span
                                                        id="price_{{ $item->product_id }}">{{ number_format($item->price * $item->quantity) }}</span>
                                                </p>
                                            </div>
                                        </div>
                                        <hr class="my-4" />
                        ...
                        <div class="card mb-4">
                            ...
                            <div class="card-body">
                                ...
                                <button class="btn btn-primary btn-lg btn-block"
                                    onclick="checkout({{ Auth::user()->id ?? null }})">
                                    Proceed to Pay
                                </button>
                            </div>
                        </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (response.success) {
                        ...
                        $('#cart-count').text(response.count <= 1 ? 'Cart - ' + response.count + ' item' :
                            'Cart - ' + response.count + ' items')
                    }
                },
                error: function(xhr, status, error) {
                    console.log(error);
