...
<div class="flex justify-center w-full">
    ...
    <div class="flex min-h-full flex-col justify-center px-6 py-12 lg:px-8 primary p-4 w-3/6 rounded-md shadow-md">
        ...
        {{-- {{ route('admin.updateProduct') }}     /admin/products/{{$product->id}}--}}
        <div class="mt-10 sm:mx-auto sm:w-full sm:max-w-sm primary">
            {{-- {{route('opportunities',['returnvalue'=> $post['id']]) }} --}}
            <form class="space-y-6" action="{{ route('admin.updateProduct', ['product' => $product['id']]) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div>
