        ...
        <div class="flex w-full">
                ...
                <form action="products">
                    <div class="relative border-2 border-gray-100 m-4 rounded-lg">
                        ...
                        <div class="absolute top-2 right-2">
                            <button type="submit"
                                class="bg-blue-500 hover:bg-blue-700 text-white font-semibold p-2 rounded mx-2 w-[80px]">
                                Search
                            </button>
                        </div>
                    </div>
            ...
            <div class="w-1/2 flex items-center justify-end">
                <form action="{{ route('admin.deleteUnusedCarts') }}" method="post">
                    @csrf
                    @method('DELETE')
                    <button class="bg-red-500 hover:bg-red-700 text-white font-semibold p-2 rounded mx-2 w-auto">Delete All
                        Unused Carts</button>
                </form>
                <a href="{{ route('admin.createProduct') }}">
                    <button class="bg-blue-500 hover:bg-blue-700 text-white font-semibold p-2 rounded mx-2">New Product
                        +</button>
                </a>
            </div>
        </div>
                    ...
                    @foreach ($products as $product)
                        <tbody class="text-center">
                            <tr>
                                ...
                                <td class="px-3 py-2 border">
                                    <a
                                        href="{{ route('admin.showProduct', ['product' => $product->id]) }}">{{ $product->name }}</a>
                                </td>
                                ...
                                <td class="px-3 py-2 border flex justify-center w-full">
                                    <a href="{{ route('admin.editProduct', ['product' => $product->id]) }}">
                                        <button
                                            class="bg-blue-500 hover:bg-blue-700 text-white font-semibold p-2 rounded mx-2 w-[80px]">Edit</button>
                                    </a>
                                    <form action="{{ route('admin.deleteProduct', ['product' => $product->id]) }}"
                                        ...
                                        @method('DELETE')
                                        <button
                                            class="bg-red-500 hover:bg-red-700 text-white font-semibold p-2 rounded mx-2 w-[80px]">Delete</button>
                                    </form>
                                </td>
                            </tr>
                        </tbody>
